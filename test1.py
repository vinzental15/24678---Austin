...
# Pyramid 
class Cropper:
    def __init__(self, x_start=20, y_start=20, x_end=50, y_end=50):
        self.root = tk.Tk()
        self.root.withdraw()
        ...
        # Set cropping coordinates if provided
        self.manual_crop = x_start is None or y_start is None or x_end is None or y_end is None
        ...
        self.newFileName = os.path.join(str(Path(self.videofile).parents[0]), str(Path(self.videofile).stem) + '_cropped1.avi')
        ...
        if self.fps == 0:
            self.fps = 30 # so change this number if cropped video has strange speed, higher number gives slower speed
...
if __name__ == "__main__":
    # Example usage with manual cropping
    app = Cropper()

    # Example usage with specified coordinates
    # app = Cropper(x_start=100, y_start=100, x_end=300, y_end=300)
